...
public:
     ...
     * @param relPos The rough position of the alignment. (Positive if r2 goes after r1 and negative otherwise)
     * @return Whether alignment succeeded
     */
    virtual bool align(const std::string &r1, const std::string &r2, ssize_t &relPos) = 0;
};
...
private:
    void stringToSortedKMers(const std::string &s, std::vector<std::pair<kMer_t, size_t>> &v);
};
