...
#pragma pack(pop)
    ...
    int paramFeatures() {
        int features =
            (paramEnableFastMode ? LIBBSC_FEATURE_FASTMODE
                                 : LIBBSC_FEATURE_NONE) |
...
#define BSC_FILEOFFSET long
    ...
    defined(__MINGW32__) || defined(__BORLANDC__) || defined(_MSC_VER)
#include <windows.h>
    double BSC_CLOCK() { return 0.001 * GetTickCount(); }
#elif defined(__unix) || defined(__linux__) || defined(__QNX__) ||             \
    defined(_AIX) || defined(__NetBSD__) || defined(macintosh) ||              \
    defined(_MAC)
#include <sys/time.h>
    double BSC_CLOCK() {
        timeval tv;
...
#endif
        {
            unsigned char *buffer = (unsigned char *)bsc_malloc(
                paramBlockSize + LIBBSC_HEADER_SIZE);
            if (buffer == NULL) {
#ifdef LIBBSC_OPENMP
...
#pragma omp master
                        ...
                        dataSize =
                            (int)fread(buffer, 1, currentBlockSize, fInput);
                        if (dataSize <= 0) {
                            fprintf(stderr, "\nIO error on file: %s!\n",
                                    argv[2]);
                            exit(1);
                        ...
                        if (paramEnableSegmentation) {
                            ...
                            if (bSegmentation) {
                                ...
                                segmentationEnd = bsc_detect_segments(
                                    buffer, dataSize, segmentedBlock, 256,
                                    paramFeatures());
                                if (segmentationEnd <= LIBBSC_NO_ERROR) {
                                    ...
                                    case LIBBSC_NOT_ENOUGH_MEMORY:
                                        fprintf(
                                            stderr,
                                            "\nNot enough memory! Please check "
                            ...
                            if (dataSize != newDataSize) {
                                BSC_FILEOFFSET pos =
                                    BSC_FTELL(fInput) - dataSize + newDataSize;
...
#pragma omp critical(print)
#endif
                        {
                            ...
                            case LIBBSC_NOT_ENOUGH_MEMORY:
                                fprintf(
                                    stderr,
                                    "\nNot enough memory! Please check README "
                    ...
                    if (recordSize > 1) {
                        int result = bsc_reorder_forward(
                            buffer, dataSize, recordSize, paramFeatures());
...
#pragma omp critical(print)
                ...
                if (paramSortingContexts == LIBBSC_CONTEXTS_AUTODETECT) {
                    sortingContexts = bsc_detect_contextsorder(buffer, dataSize,
                                                               paramFeatures());
...
#pragma omp critical(print)
                ...
                if (sortingContexts == LIBBSC_CONTEXTS_PRECEDING) {
                    int result =
                        bsc_reverse_block(buffer, dataSize, paramFeatures());
                    if (result != LIBBSC_NO_ERROR) {
#ifdef LIBBSC_OPENMP
#pragma omp critical(print)
#endif
                ...
                int blockSize = bsc_compress(
                    buffer, buffer, dataSize, paramLZPHashSize, paramLZPMinLen,
...
#pragma omp critical(input)
                    ...
                    blockSize =
                        bsc_store(buffer, buffer, dataSize, paramFeatures());
                }
...
#pragma omp critical(print)
#endif
                    {
                        ...
                        case LIBBSC_NOT_SUPPORTED:
                            fprintf(stderr,
                                    "\nSpecified compression method is not "
                                    "supported on this platform!\n");
                            break;
                        case LIBBSC_GPU_ERROR:
                            fprintf(stderr,
                                    "\nGeneral GPU failure! Please check "
                                    "README file for more information.\n");
                            break;
                        ...
                        case LIBBSC_GPU_NOT_ENOUGH_MEMORY:
                            fprintf(stderr,
                                    "\nNot enough GPU memory! Please check "
                                    "README file for more information.\n");
                            break;
...
#endif
                {
                    BSC_BLOCK_HEADER header = {blockOffset, recordSize,
                                               sortingContexts};

                    if (fwrite(&header, sizeof(BSC_BLOCK_HEADER), 1, fOutput) !=
                        1) {
                        fprintf(stderr, "\nIO error on file: %s!\n", argv[3]);
                        exit(1);
                    }

                    if ((int)fwrite(buffer, 1, blockSize, fOutput) !=
                        blockSize) {
                        fprintf(stderr, "\nIO error on file: %s!\n", argv[3]);
                        exit(1);
                    }
...
#pragma omp master
                        ...
                        if (fread(&header, sizeof(BSC_BLOCK_HEADER), 1,
                                  fInput) != 1) {
                        ...
                        if (recordSize < 1) {
                            fprintf(stderr,
                                    "\nThis is not bsc archive or invalid "
                                    "compression method!\n");
                            exit(2);
                        }
                        ...
                        if (fread(bscBlockHeader, LIBBSC_HEADER_SIZE, 1,
                                  fInput) != 1) {
                            fprintf(stderr, "\nUnexpected end of file: %s!\n",
                                    argv[2]);
                            exit(1);
                        ...
                        if (bsc_block_info(bscBlockHeader, LIBBSC_HEADER_SIZE,
                                           &blockSize, &dataSize,
                                           paramFeatures()) !=
                            LIBBSC_NO_ERROR) {
                            fprintf(stderr,
                        ...
                        if (fread(buffer + LIBBSC_HEADER_SIZE,
                                  blockSize - LIBBSC_HEADER_SIZE, 1,
                                  fInput) != 1) {
                            fprintf(stderr, "\nUnexpected end of file: %s!\n",
                                    argv[2]);
                            exit(1);
...
#pragma omp critical(output)
    ...
    void ProcessSwitch(char *s) {
        ...
        for (; *s != 0;) {
...
#ifdef LIBBSC_SORT_TRANSFORM_SUPPORT
                case 3:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST3;
                    break;
                case 4:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST4;
                    break;
                case 5:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST5;
                    break;
                case 6:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST6;
                    break;
                case 7:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST7;
                    paramEnableCUDA = 1;
                    break;
                case 8:
                    paramBlockSorter = LIBBSC_BLOCKSORTER_ST8;
                    paramEnableCUDA = 1;
                    break;
#endif
            ...
            case 'p':
                paramEnableLZP = paramEnableSegmentation =
