...
NanoporeReads::NanoporeReads(const char *fileName, int k, int n)
    : k(k), n(n), sketches(NULL) {
...
void NanoporeReads::calculateMinHashSketches() {
    ...
    kMer_t *kMers;
    // Because of memory constraints on the GPUs we cannot deal with all the
    // reads at once. So we arrange the reads into blocks of blockSize reads and
    // only work on a single block at the same time.
    ...
    for (size_t currentRead = 0; currentRead < numReads;
         currentRead += blockSize) {
        std::cout << "CurrentRead " << currentRead << std::endl;
        const size_t readsLeft = numReads - currentRead;
        const size_t currentBlockSize =
            readsLeft > blockSize ? blockSize : readsLeft;

        auto generateKMers = [&]() {
            //            cudaMallocManaged(&kMers, currentBlockSize * numKMers
            //            * sizeof(kMer_t));
            kMers =
                (kMer_t *)malloc(currentBlockSize * numKMers * sizeof(kMer_t));
            for (size_t i = 0; i < currentBlockSize; i++) {
                size_t baseIndex = i * numKMers;
#pragma omp parallel for
                for (size_t index = baseIndex; index < numKMers + baseIndex;
                     index++) {
                    kMers[index] = kMerToInt(readData[i + currentRead]->substr(
                        index - baseIndex, k));
                }
            }
        };
        ...
        kMer_t *hashes;
        //        cudaMallocManaged(&hashes, n * currentBlockSize * numKMers *
        //        sizeof(kMer_t));
        hashes =
            (kMer_t *)malloc(n * currentBlockSize * numKMers * sizeof(kMer_t));
#ifdef _GPU
        const size_t blockSize = 512;
        const size_t numBlocks = 512;
        hashKMer_GPU<<<numBlocks, blockSize>>>(currentBlockSize * numKMers, n,
                                               kMers, hashes, randNumbers);
...
#else
        {
            ...
            std::cout << duration.count() << " milliseconds passed"
                      << std::endl;
        }
#endif
        ...
        // Now we are going to compute the sketches which are the minimums of
        // the hashes
#ifdef _GPU
        calcSketch_GPU<<<(currentBlockSize + blockSize - 1) / blockSize,
                         blockSize>>>(currentBlockSize, currentRead, numKMers,
                                      n, hashes, sketches, kMers);
        cudaDeviceSynchronize();
#else
        {
            auto start = std::chrono::high_resolution_clock::now();
...
void NanoporeReads::hashKMer(const size_t numReads, const size_t numKMers,
    ...
    for (size_t i = index; i < numReads; i += stride) {
        ...
        for (size_t j = 0; j < n; ++j) {
            ...
            for (size_t l = 0; l < numKMers; ++l) {
                ...
                minIndex = currentMin < temp ? minIndex : l;
                currentMin = currentMin < temp ? currentMin : temp;
                //                currentMin = temp ^ ((currentMin ^ temp) &
                //                -(currentMin < temp));
            }
            // std::cout << "thread: " << i << " hash id: " << j << std::endl;
            // std::cout << "minIndex " << minIndex << ":" << kMers[i * numKMers
            // + minIndex];
...
filterStats NanoporeReads::getFilterStats(unsigned int overlapBaseThreshold,
...
MinHashReadFilter::MinHashReadFilter(size_t overlapSketchThreshold,
                                     NanoporeReads &nR)
